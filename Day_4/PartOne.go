...
func main() {
	...
	for _, e := range guardEvents {
		...
		case "wakes":
			...
			if timeSlept < totalSleep {
				mostSleptGuard = currentGuard
				timeSlept = totalSleep
			}
	...
	guardSleepTime := resultMap[mostSleptGuard].sleepPerMin
	mostSleptTime := 0
	timeOfMostSlept := 0
	for i := 0; i < 60; i++ {
		if guardSleepTime[i] > mostSleptTime {
			timeOfMostSlept = i
			mostSleptTime = guardSleepTime[i]
		}
	}

	fmt.Println(mostSleptGuard)
	fmt.Println(timeSlept)
	fmt.Println(timeOfMostSlept)
}
